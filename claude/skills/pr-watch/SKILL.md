...
リポジトリが対応環境マトリクスと非ゴールを宣言している場合、各指摘を
**マトリクス内で到達可能で、かつ明示された非ゴールに当たらないか** で分類する。
到達可能性だけでは足りない — 非ゴールは定義上到達可能なので、一軸で見ると
必ず修正対象になってしまう。

**マトリクスは PR の branch 側からは読まない。** リポジトリのエージェント指示
ファイル(fanout では `AGENTS.md` の `Automated PR Review Scope`)を **PR の base**
で参照する。PR 自身が書き換えた scope 文書を信頼すると、その PR が本当は
actionable な指摘へ「範囲外」と返信して repair を終えられてしまう。

| 分類 | 対応 |
| --- | --- |
| 到達可能 かつ 非ゴール外 | 修正する。重大度(P1 / P2 等)では落とさない |
| 到達不能 または 非ゴール | 修正せず、thread に理由と該当する scope 行を書いて返信する |
| 判断が割れる / PR のスコープを超える | 止めてユーザーに確認する。自分だけで決めない |

無言で直す・無言でクローズするのはどちらも禁止。裁定を書き残さないと、
次のレビューラウンドで同じ指摘が戻ってくる。

到達可能な欠陥を自分の判断で後回しにしない。follow-up issue に切り出すかは
ユーザーの判断であり、決まるまで PR は「未完了」のままにする。既知の
到達可能な欠陥を抱えたまま完了扱いにしてはいけない。

そのラウンドの指摘が範囲外のものだけだったら、修正せず返信して
レビュー対応を終える。範囲外の指摘を直すとコードが増え、次ラウンドの
レビュー面が広がってラウンドが収束しなくなる。

スコープ宣言が無いリポジトリでは従来どおり全件を検討してよいが、
...
review 指摘が 3 pass 連続で新規に出続け、かつ裁定で範囲外に落ちる割合が
