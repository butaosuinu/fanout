...
- **Forward extra flags** (`--agent`, `--limit`, `--only`, `--skip`, `--include`, `--name`, `--session`, `--sleep`, `--popup-timeout`) verbatim to both the dry-run and the real run. Strip `--go` before forwarding — it is the slash command's own flag, not a `fanout` flag.
