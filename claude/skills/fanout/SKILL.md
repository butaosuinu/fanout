...
- **Forward extra flags** (`--agent`, `--limit`, `--only`, `--skip`, `--include`, `--unblocked-only`, `--status`, `--name`, `--session`, `--sleep`, `--popup-timeout`) verbatim to both the dry-run and the real run. Strip `--go` before forwarding — it is the slash command's own flag, not a `fanout` flag.
...
- `--status <name>` (**project mode only**) filters Projects v2 items by their `Status` single-select field. Default is `Todo` — so `fanout <project-url>` with no other flags fans out only the Todo column. Pass `--status all` to disable the filter and include every OPEN item in the Project. Pass any single Status value (e.g. `--status "In Progress"`, `--status Backlog`) to target that column; the value is matched against the Project's Status field options case-sensitively. If the Project has no `Status` field at all, `fanout` warns and falls back to all OPEN items. Silently ignored when the positional arg is an issue number.
...
- **URL shape** — accepted forms are `https://github.com/users/<owner>/projects/<num>` and `https://github.com/orgs/<owner>/projects/<num>` (with optional trailing `/`). Both user-owned and organization-owned Projects v2 boards are supported. Anything else is rejected at arg-parse time.
...
- **`--status` filtering** — see the `## Running` section above. The default is `Todo`, which mirrors the common "queue everything I'm planning to start" workflow. Use `--status all` for a full fan-out, or a single explicit value for any other column.
...
- Project mode `no items on Project <url>` (or "no items matching --status <name>") is not a failure; fanout exits 0.
