...
- `--codex-plan-mode` / `--no-codex-plan-mode` apply only when every selected child resolves to `codex`. TUI-created manual `codex` panes use the same app-server Plan Mode path automatically instead of writing a briefing file. The prompt keeps normal non-mutating discovery before the `<proposed_plan>` response. When enabled, fanout starts a Codex app-server, creates a Plan Mode turn with the child prompt, then attaches the interactive Codex TUI. If app-server Plan Mode startup or TUI attach fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
