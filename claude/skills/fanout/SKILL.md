...
- `--codex-plan-mode` / `--no-codex-plan-mode` apply only when every selected child resolves to `codex`. TUI-created manual `codex` panes use the same Plan Mode path automatically but type the popup prompt into `/plan` instead of writing a briefing file. The prompt keeps normal non-mutating discovery before the `<proposed_plan>` response. When enabled, fanout starts a Codex app-server, launches an interactive Codex TUI, and types the child prompt into the TUI through `/plan`. If `/plan` thread startup or TUI attach fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
