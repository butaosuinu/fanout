...
4. **Agent name is required** — pass `--agent claude` / `--agent codex`, or set `FANOUT_AGENT`. When this skill runs fanout and the user did not provide an agent, add `--agent claude`. `--codex-plan-mode` is valid only with `--agent codex`; it uses Codex app-server to create the child Plan Mode thread, start the initial Plan turn with the fanout prompt, then attach the interactive Codex TUI to that remote session.
