...
- `--codex-plan-mode` / `--no-codex-plan-mode` apply only when every selected child resolves to `codex`. TUI-created manual `codex` panes use the same Plan Mode path automatically but pass the modal prompt inline instead of writing a `/tmp` briefing file. When enabled, fanout starts a Codex app-server, creates the child Plan Mode thread, starts the initial Plan turn with the child prompt through app-server, and attaches the interactive Codex TUI to that remote session. fanout does not send `/plan` or prompt text through tmux. If Plan turn setup or TUI attach fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
