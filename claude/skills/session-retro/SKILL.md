...
- **既知の限界**: 前回スナップショットのいずれかのメトリクスが `truncated=true` だった場合でも、`window.until` はそのまま次回の `SINCE` になる。つまり収集が不完全だった期間の tool error / CI 失敗 / レビューコメントは、次回以降二度と取得されない (Step 5 で truncated なメトリクスを改善/再発判定から除外しても、window 自体は先に進んでしまう)。メトリクスごとに独立した window を持つ設計が必要で、この skill のスコープを超える — フォローアップ #394。
