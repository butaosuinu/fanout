...
- `--team` is forwarded like the other briefing flags (default off). It opts the run into sibling-pane peer messaging: each child briefing gets a "Coordinating with your sibling panes" section and the created panes are seeded into a per-parent SQLite peer registry (best-effort; a registry failure never fails the fan-out). Suggest it when the children will touch shared files or have ordering dependencies not already encoded as blockers. The `fanout msg` subcommand (`peers` / `inbox` / `board` / `send` / `post` / `mark-read` / `register`) is the runtime channel, but it is **not** part of this slash command's forwarding — each fanned agent runs `fanout msg` directly from inside its own pane; see the fanout skill (`~/.claude/skills/fanout/SKILL.md`, "Sibling coordination") for the verb surface.
