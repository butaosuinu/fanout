...
modal and create one or more prompt-based `claude` / `codex` / `opencode` panes from the
...
1. **Resolve the parent target** from `$ARGUMENTS`. Two input shapes are accepted; the first matching token wins:
   ...
   - This fallback is slash-command/skill-side target resolution for non-TTY agent entrypoints. Do not change the Go `fanout` CLI for it; the CLI already accepts the resolved positional arg via `internal/app/cliflags.Parse()`.
...
10. **On failure**: consult `/Users/butaosuinu/fanout/README.md` Troubleshooting and surface the most likely fix. Common cases:
   ...
   - `agent is required` → rerun with `--agent claude`, `--agent codex`, `--agent opencode`, set `FANOUT_AGENT`, or cover every selected child with `--agent NUM=name`.
...
- Settings flags are forwarded like other fanout flags: `--auto-pr` / `--no-auto-pr`, `--pr-review-gate` / `--no-pr-review-gate`, `--briefing-code-review` / `--no-briefing-code-review`, `--agent-teams-hint` / `--no-agent-teams-hint`, and `--pr-visualization` / `--no-pr-visualization`. `--pr-visualization` controls structured PR-body plus gated Mermaid briefing guidance; briefing settings default on. Lifecycle hooks are always on and come from user `hooks.json`.
- `--team` is forwarded like the other briefing flags (default off). It opts the run into sibling-pane peer messaging: each child's standard briefing gets a "Coordinating with your sibling panes" section and the created panes are seeded into a per-parent SQLite peer registry (best-effort; a registry failure never fails the fan-out). Codex Plan Mode children (`--codex-plan-mode`) get the minimal Plan briefing, so the section is skipped for them — they are still seeded and can run `fanout msg`. Suggest it when the children will touch shared files or have ordering dependencies not already encoded as blockers. The `fanout msg` subcommand (`peers` / `inbox` / `board` / `watch` / `send` / `post` / `nudge` / `mark-read` / `register`) is the runtime channel, but it is **not** part of this slash command's forwarding — each fanned agent runs `fanout msg` directly from inside its own pane; `claude` children's briefings additionally instruct starting `fanout msg watch` under the Monitor tool, so sibling messages stream in as they arrive. See the fanout skill (`~/.claude/skills/fanout/SKILL.md`, "Sibling coordination") for the verb surface.
- `--codex-plan-mode` is forwarded like other fanout flags and is valid only when every selected child uses `codex` after per-issue `--agent NUM=name` overrides are applied. fanout starts a child Codex app-server, creates a Plan Mode thread, attaches the normal interactive Codex TUI, and starts the fanout prompt. It records the launch after the initial Plan turn is accepted; slow plan generation or approval waiting never triggers startup cleanup. Failures before app-server startup, TUI attach, thread setup, or initial turn acceptance still fail the launch and clean up the pane/worktree so the child can be retried.
...
- Always pass an agent explicitly unless the user has already supplied `FANOUT_AGENT`; use `--agent codex` when `--codex-plan-mode` is present, otherwise choose the requested/default agent such as `--agent claude`. Per-issue overrides use repeatable `--agent NUM=name` flags and are merged with the global default.
...
- `/fanout 123 --agent codex --agent 456=claude` — launch most children with Codex, but child issue #456 with Claude.
- `/fanout 123 --agent codex --codex-plan-mode` — launch Codex child panes as interactive app-server-backed Plan Mode TUI sessions.
