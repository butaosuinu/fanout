...
1. **Resolve the parent target** from `$ARGUMENTS`. Two input shapes are accepted; the first matching token wins:
   ...
   - **Project mode** — first token matching `^https://github\.com/(users|orgs)/[^/]+/projects/\d+/?$` → pass the URL to the CLI verbatim as the positional arg (no normalization, the CLI parses `users` vs `orgs` itself).
...
4. **Generate pane names for each target** — dmux's default slug generator would otherwise call OpenRouter / the local `claude --no-interactive` fallback just to name each pane. In issue mode you already have every target's title and body from the parent issue context; in project mode pull the target numbers and titles from the first dry-run output (step 5) and, if you need body content for naming, fetch each per-issue body with `gh issue view <num> --json body -q .body`. Produce a `<slug-hint>` (2–4 kebab-case words) and a `<display-name>` (≤40 readable chars, JP/EN OK) per target. Forward them as `--name <NUM>=<slug-hint>|<display-name>` (one flag per target, repeatable). See `~/.claude/skills/fanout/SKILL.md` "Generate pane names" for the naming policy. Do not ask the user to confirm the names individually — they'll see them in the dry-run summary and can course-correct then.
5. **Dry-run first** (unless `--go` was passed):
   - Run `fanout <target> --dry-run <forwarded>` (where `<target>` is the issue number or Project URL from step 1; include any `--include` from step 3 and `--name` from step 4) via Bash. cwd is irrelevant; do NOT `cd` first.
...
- `--status <name>` (**project mode only**): filter Project items by their Status field value. Default is `Todo` (i.e. `/fanout <url>` fans out only the Todo column). Pass `--status all` to disable the filter and fan out every OPEN item in the Project. Pass any single Status value (e.g. `--status "In Progress"`) to target that column. The Status field is matched case-sensitively. If the Project has no Status field at all, `fanout` warns and falls back to all OPEN items. Silently ignored in issue mode.
...
- `/fanout https://github.com/users/butaosuinu/projects/3 --status all` — project mode, disable the Status filter, fan out every OPEN item.
- `/fanout https://github.com/orgs/acme/projects/12 --status "In Progress" --limit 5` — organization Project, In Progress column, first 5 items only.
