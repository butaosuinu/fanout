...
- `--codex-plan-mode` is forwarded like other fanout flags and is valid only with `--agent codex`. It opens the normal interactive Codex TUI, switches it with `/plan`, and submits the fanout prompt. If Codex is not visibly ready or Plan Mode is not visibly confirmed, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
...
- `/fanout 123 --agent codex --codex-plan-mode` — launch Codex child panes as interactive TUI sessions, switch them to `/plan`, and submit the fanout prompt.
