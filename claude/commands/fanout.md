...
- `--codex-plan-mode` is forwarded like other fanout flags and is valid only when every selected child uses `codex` after per-issue `--agent NUM=name` overrides are applied. fanout starts a child Codex app-server, creates a Plan Mode thread, attaches the normal interactive Codex TUI, then starts the fanout prompt and waits for the approval UI. The prompt tells Codex to inspect relevant context before presenting a plan. If app-server Plan Mode startup, TUI attach, or approval UI readiness fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
