...
7. **Optional wait loop**:
   - Only when `--wait` was present for issue mode, follow the skill doc's
     "Optional Wait-And-Continue" section. If `--wait` was passed with a
     Project URL, explain that the wait loop currently requires a parent issue.
