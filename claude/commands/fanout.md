...
5. **Generate pane names for each target** — dmux's default slug generator would otherwise call OpenRouter / the local `claude --no-interactive` fallback just to name each pane. In issue mode you already have every target's title and body from the parent issue context; in project mode pull the target numbers and titles from the first dry-run output (step 6) and, if you need body content for naming, fetch each per-issue body with `gh issue view <num> --json body -q .body`. Produce a `<slug-hint>` (2–4 kebab-case words) and a `<display-name>` (≤40 readable chars, JP/EN OK) per target. Forward them as `--name <NUM>=<slug-hint>|<display-name>` (one flag per target, repeatable). See `~/.claude/skills/fanout/SKILL.md` "Generate pane names" for the naming policy. Do not ask the user to confirm the names individually — they'll see them in the dry-run summary and can course-correct then.
