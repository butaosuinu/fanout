...
- `--codex-plan-mode` is forwarded like other fanout flags and is valid only when every selected child uses `codex` after per-issue `--agent NUM=name` overrides are applied. fanout starts a child Codex app-server, launches the normal interactive Codex TUI, and types the fanout prompt through `/plan` in the composer. The prompt tells Codex to inspect relevant context before presenting a plan. If `/plan` thread startup or TUI attach fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
