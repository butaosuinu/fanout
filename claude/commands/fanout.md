...
2. **Detect `--go`** in the remaining arguments. Strip it out — it is this command's own bypass flag, not a `fanout` flag and not a selector for the Go implementation. The rest of the arguments are forwarded verbatim to `fanout`. If neither the user nor the environment supplies an agent, add `--agent claude` because the direct tmux runtime requires an explicit agent name.
3. **Reject `--wait` for now** if it appears in the remaining arguments. Strip it out, tell the user that `/fanout --wait` is temporarily unsupported while action mode writes no state for `fanout --status`, and stop before dry-run or execution. Do not start a polling loop for new direct tmux runs until the state-store phase lands.
...
- Rerun is safe for existing worktree directories; action mode skips children when the exact `.fanout/worktrees/<slug>` exists or another generated worktree directory ends in `-<issue-number>`. `fanout --status` still reads legacy dmux state until the state-store phase lands.
...
- `--wait` is temporarily unsupported in direct tmux action mode. Do not start the historical `fanout --status` polling loop for new runs until the state-store phase lands.
