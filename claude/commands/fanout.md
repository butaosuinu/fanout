...
- `--codex-plan-mode` is forwarded like other fanout flags and is valid only when every selected child uses `codex` after per-issue `--agent NUM=name` overrides are applied. fanout starts a child Codex app-server, creates a Plan Mode thread, and resumes it with the fanout prompt through the normal interactive Codex TUI. If Plan Mode thread setup or TUI attach fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
