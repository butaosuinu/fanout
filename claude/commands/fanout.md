...
3. **Reject `--wait` for now** if it appears in the remaining arguments. Strip it out, tell the user that `/fanout --wait` is temporarily unsupported while `fanout --status` still reads legacy dmux state instead of `.fanout/state.json`, and stop before dry-run or execution. Do not start a polling loop for new direct tmux runs until `--status` reads the fanout state store.
...
- Rerun is safe for recorded panes; action mode skips children already present in `.fanout/state.json` for the same `(parent, issueNum)`, and also skips unrecorded existing `.fanout/worktrees/<slug>` directories as a migration fallback. If the same issue is recorded for another parent, only an existing worktree matching the slug this current run would create is treated as fallback. `fanout --status` still reads legacy dmux state until its own migration lands.
...
- `--wait` is temporarily unsupported in direct tmux action mode. Do not start the historical `fanout --status` polling loop for new runs until `--status` reads `.fanout/state.json`.
