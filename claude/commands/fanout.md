...
- `--codex-plan-mode` is forwarded like other fanout flags and is valid only when every selected child uses `codex` after per-issue `--agent NUM=name` overrides are applied. fanout starts a child Codex app-server, creates a Plan Mode thread, and resumes it with the fanout prompt through the normal interactive Codex TUI. The prompt tells Codex to inspect relevant context before presenting a plan. If Plan Mode thread setup or TUI attach fails, fanout fails that launch before recording state and cleans up the pane/worktree so the child can be retried.
...
- `/fanout 123 --agent codex --codex-plan-mode` — launch Codex child panes by creating app-server Plan Mode threads and resuming interactive TUI sessions with the child prompts.
