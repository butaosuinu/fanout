...
3. **Detect `--wait`** in the remaining arguments. Strip it out — it is template-side only and is never forwarded to the `fanout` CLI. `--wait` is meaningful only for issue-mode pane creation, not Project URLs, `--dry-run`, `--status`, `--close`, `--merge`, or `--cleanup`. If neither the user nor the environment supplies an agent for a pane-creation run, add `--agent claude`, because the direct tmux runtime requires an explicit agent name. Forward any user-supplied repeatable `--agent NUM=name` flags verbatim; supported agents in this build are `claude`, `codex`, and `opencode`, so do not emit `gemini`.
...
- Always pass an agent explicitly unless the user has already supplied `FANOUT_AGENT`; choose the requested/default agent such as `--agent claude`. Per-issue overrides use repeatable `--agent NUM=name` flags and are merged with the global default. Plan Mode is resolved separately from the user-level `childPlanMode` setting or `FANOUT_CHILD_PLAN_MODE`.
