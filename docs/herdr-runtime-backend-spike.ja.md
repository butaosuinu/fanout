...
- tracked の変更または非 ignored の untracked file が残る checkout は削除しない。ignored file だけなら clean と同等に扱い、identity の照合後に `git clean -fdX` で除去し、削除件数を 1 行記録してから force なしの `herdr worktree remove` へ進む。埋め込み Git repository などが残る場合は停止し、手動 cleanup を要求する。branch は herdr に削除させない。
