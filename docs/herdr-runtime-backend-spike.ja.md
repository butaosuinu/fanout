...
Herdr backend の `--team` は #568 の registry-backed peer 解決を使い、issue / Project / plan の launch で受理する。
#552 の Herdr nudge 実装は registry-backed の宛先解決を使い、送信前の live identity gate を追加する。
...
| `--team` | Go | final row から peer registry を seed し、issue / Project / plan の自己識別と push lane を backend 共通で処理する。`--dry-run` は registry を変更しない |
...
| console / coordinator close | Go | plan coordinator は `fanout plan <slug> --cleanup` で task 行が 0 件になった後に退役する。checkout を持たない exact owned workspace / pane を送信直前に再照合し、同じ workspace に別 pane が残れば mutation せず manual cleanup にする。応答喪失は再実行時の存在確認で確定する |
...
各 intent 行は起動元の physical worktree root と task provenance を保持する。final row は owning worktree の `.fanout/state.json` pane row として確定し（tmux backend と同じ所在）、intent 行は journal の外へ複製しない。
team 共通経路は final row の backend と typed key を保ったまま Herdr row を peer registry へ渡す。
...
owned config は `[update] manifest_check = false` を固定して background remote manifest check を無効にし、`[terminal] default_shell` を絶対 fanout launcher path、`shell_mode` を `non_login` に固定する。
...
registry-backed peer の nudge 宛先は、current recipient row を owning worktree の `state.json` から解決する。
...
- compare-and-delete は fanout の branch 削除手順であり、照合済み tip（launch rollback は記録済み base SHA、cleanup は保存済み checkout の照合後かつ worktree mutation 前の current tip）との一致と、同じ full branch ref を checkout する linked worktree の不在を確認してから削除する。cleanup の `CloseEverything` は fanout が作成した branch だけを対象とし、tip が一致すれば未マージ commit を持つ branch も削除する。fresh cleanup の開始時または planned intent の再計画時に保存済み checkout が既に不在なら、同名 branch の ownership を確認できないため残す。checkout を検査しない `update-ref -d` 単独では行わず、checked-out branch を拒否する tmux backend の `git branch -D` と同じ guard を保つ。
- `worktree create` 成功後の launch 失敗（launcher timeout / exit、token 失敗、agent 検出失敗）は tmux backend の `failCleanup` と同水準で rollback する。rollback は launch cycle と別の operation として新しい intent 行と budget で実行し、保存済み label nonce / branch / path / `terminal_id` を再照合してから force なしの `worktree remove` を一回発行し、workspace / checkout の不在を確認できた場合だけ自作 branch を記録済み base SHA の compare-and-delete で削除して両 intent 行を消す。照合不一致と dirty 拒否では資源を残して `manual_cleanup_required` にする（tmux も close を確認できない場合は worktree を温存する）。crash 後に rollback intent が `planned` なら未発行として元の realized intent を復元する。`issued` なら remove を再発行せず、workspace / checkout の不在を確認できた場合だけ rollback 完了として branch 削除と intent 整理へ進み、残存または判定不能では旧 request の進行中を排除できないため `manual_cleanup_required` にする。
...
- token の `pane run` も socket mutation として扱い、発行直前に intent 行へ token 発行済み flag を state save する。flag が立った後の timeout、cancellation、error、response loss は、同名 agent や一致する child process chain を観測できても自動採用せず `manual_cleanup_required` にする。token は再送しない。flag がない場合だけ fresh marker の再観測後に token を発行できる。
...
launcher は exact checkout cwd、`HERDR_PANE_ID=w3:p1`、`HERDR_WORKSPACE_ID=w3`、operation nonce を受け、`pane wait-output` は `FANOUT_HERDR_READY:<nonce>` を検出した。
...
env file は private namespace gate 済みの 0700 directory 配下に exclusive create し、intent 行には path と name 数だけを記録して raw value を保存しない。capsule は書き込み前に 64 KiB 以下であることを確認し、reader と同じ上限を使う。
...
| #552 | #568 の registry-backed peer 登録 / 自己識別 / 宛先解決を使い、live `pane process-info` / OS process identity と final state を送信直前に再照合した exact pane ID へ no-wait の `agent prompt` nudge を発行する |
| #554 | owned launcher 経由の `fanout __codex-plan-tui` controller 起動と controller / Codex child の process 照合 |
| #568 | issue / plan cohort の peer 登録、plan lane の preseed / cleanup、自己識別、宛先解決、Claude / Codex push caller を `state.json` の canonical Herdr pane row へ移行し、`--team` を解禁する |
...
token 発行後の結果が確定しない場合も再送せず、row を `stale` のまま残す。
...
wave 2 の Herdr backend は registry-backed peer 解決を使って `--team` を受理する。
`internal/app/run/team.go` は `state.json` の Herdr issue / plan task row から cohort と `TaskID` を作り、parent ごとの SQLite registry へ seed する。
`internal/app/run/task_team_registry.go` の `preseedTaskTeamRegistry` / `cleanupUncreatedTaskPeers` は、最初の Codex pane より前の provisional plan cohort と fail-fast 後の未作成 peer cleanup を処理する。
作成済み task の final Herdr row が provisional peer を置換した後は、cleanup がその peer を削除しない。
`internal/infra/team/detect.go` の自己識別は worktree path から canonical Herdr row を選び、typed row key、pane identity、issue number または `TaskID` を解決する。
...
pre-token recovery は保存済み team mode、正規化済み DB path、Codex bridge mode と再実行 request の一致を検証し、不一致なら token 発行前に拒否する。
...
#552 の Herdr 実装は前述の registry-backed peer 解決を使い、送信前の identity gate を追加する。
...
Herdr backend の `--team` は message bus と Claude / Codex push lane を開始する。
自動 nudge の送信は #552 まで no-op とし、herdr の `notification show` も nudge に使わない。
...
| console / coordinator close | Go | plan coordinator は `fanout plan <slug> --cleanup` で task 行が 0 件になった後に退役する。checkout を持たない exact owned workspace / pane を送信直前に再照合し、同じ workspace に別 pane が残れば mutation せず manual cleanup にする。応答喪失は再実行時の存在確認で確定する | close が authoritative server generation と target resource generation を原子的に検査する |
...
| `--team` | Go | issue / plan の final row を parent ごとの registry へ seed し、plan preseed / cleanup、自己識別、宛先解決、Claude / Codex push caller を backend 共通で処理する。`--dry-run` は変更を行わない | message bus の server-side tenant key |
...
| AgentID 再利用 fence の per-snapshot `ProcessIdentity` 確証 | 据え置き。2026-09-05 JST に XDG 四 root と socket を `/private/tmp` へ隔離した herdr 0.8.2 で、名前付き Codex を `agent start` して `ctrl+c` で終了後、同じ pane、`terminal_id`、name への別 Codex の `agent start` が成功した。別 process が foreground を占有中だけ `agent_pane_busy` になり、shell 復帰型 pane の AgentID 再利用は防がない。一方、fanout-owned agent workload は launcher を `syscall.Exec` で置換するため、agent exit 時に pane が消えて shell へ戻る窓がない。shell handoff は console workload だけに許し、その row は AgentID で同定しない。process を新たに束縛する launch / cold-resume と、`agent.prompt` を発行する automatic nudge の mutation 境界では `ProcessIdentity` を採取、照合済みであり、per-snapshot 確証は pane ごとの `pane process-info` を各 poll に追加するため採用しない。設計制約として、fanout-owned agent の exit 後に shell へ戻すか pane を再利用可能なまま残す変更、または shell handoff row を AgentID で同定する変更では必ず再評価する。herdr が `session.snapshot` に process identity を載せた場合も再評価する |
