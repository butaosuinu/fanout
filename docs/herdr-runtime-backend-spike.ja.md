...
- compare-and-delete は fanout の branch 削除手順であり、照合済み tip（launch rollback は記録済み base SHA、cleanup は worktree mutation 前の current tip）との一致と、同じ full branch ref を checkout する linked worktree の不在を確認してから削除する。cleanup の `CloseEverything` は fanout が作成した branch だけを対象とし、tip が一致すれば未マージ commit を持つ branch も削除する。checkout を検査しない `update-ref -d` 単独では行わず、checked-out branch を拒否する tmux backend の `git branch -D` と同じ guard を保つ。
...
- branch 削除は fanout の compare-and-delete（「safety gate」節で定義。tip 照合と checked-out worktree 不在の確認後の削除）だけが行う。`CloseEverything` は fanout が作成した branch の worktree mutation 前の tip を記録し、merge の有無を条件にせず、一致時だけ削除する。事前存在した branch は自動削除しない。
