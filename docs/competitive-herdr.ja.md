...
作者自身の `herdr-plugin-github-start`(GitHub issue / PR / discussion から agent 起動。issue body 非取得・worktree 非作成・fan-out なし)、GitHub issue 選択 → `gh issue develop` → worktree / workspace 作成 → ローカル差分 review(lazygit)・commit push・PR の view / create まで一本で提供する `herdr-plugin-gh-workflow`(GitHub 上の PR review 投稿は持たない)、カンバンカードを prompt として agent に配り自動 Plan → Execute → Review と手動 gate を持つ `herdr-board`(`herdr-kanban` はタスクと tab の紐付けのみで agent 起動なし)、Claude Code teams の `herdmates`、Linear issue → worktree の `herdr-worktree-from-linear` と GitHub PR → worktree の `herdr-worktree-from-pr`、スケジュール起動の `herdr-routines`、merge 検知 branch cleanup の `herdr-branch-cleanup`、レビュー・PR 追跡の `herdr-reviewr` / `herdr-pr-tracker`、人間承認ゲートの `herdr-approval-gate`。
