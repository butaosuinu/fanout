...
コーディングエージェントの価値はモデル単体ではなく、その周りの運用基盤 — ツールオーケストレーション・検証ループ・コンテキスト・ガードレール・可観測性の 5 層 — に移っている。fanout はオーケストレーション（tmux + worktree 隔離、wave、team msg）、検証ループ（post-work-review gate、pr-watch）、コンテキスト（issue 本文からの briefing 生成）、ガードレール（fail-fast、localhost 限定 dashboard、opt-in watcher）の 4 層が既に強く、**可観測性（トークン/コスト計測が無い）だけが弱い**。層の外にもう 1 つ、着信イベント（レビューコメント・blocker 解消・トリガーラベル）に応答してループを閉じる仕組みも薄い。着手順はこの 2 つを埋める順に組む。
...
| 週 2 | 〜2026-07-15 | #361 完遂（#358-360）/ #368 wave 1-2（#790 #362 #363 → #364 #365 #791 #792）/ #313 wave2（#307-309）/ #106 レビュー追従 |
| 週 3 | 〜2026-07-22 | #368 完遂（wave 3-4: #366 → #367。順序は [model-effort-selection.ja.md](model-effort-selection.ja.md)）/ #373 コア（#369-371）/ #313 完遂（#310-312）/ #175 リスクレーン（#176-179） |
| 週 4 | 〜2026-07-29 | #180 読み順（#181-182）/ #107 + #292 自動トリガー / #373 docs（#372）/ #241 cursor/copilot（#242, #244-247）/ #59 Wave 自動進行 |
| Icebox | — | #183-187（v2 後段）/ #58 Linear / #9 Windows。小粒は随時: #13 #14 #254 #8 #7。#321 Wails アプリは [dashboard-roadmap.ja.md](dashboard-roadmap.ja.md) の phase 4（#659 完了後・任意）へ移動 |
