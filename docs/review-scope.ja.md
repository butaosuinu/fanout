...
基準は 2 つ。**対応環境マトリクス内で到達可能な trigger を示せるか**、そして
**上の「やらないこと」に当たらないか**。到達可能性だけで判定してはいけない —
非ゴール(他プロセスとの完全排他、crash-resume の全窓回収)は定義上マトリクス内で
到達可能なので、一軸で見ると必ず修正対象になり、非ゴール宣言と矛盾する。

1. **両方を満たす** → 修正する。重大度(P1 / P2)では落とさない。
   マトリクス内で踏めて非ゴール外なら P2 でも直す
2. **どちらかを外す** → thread に理由を書いて decline する。
