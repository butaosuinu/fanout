...
ただし畳む条件に正規化前の name の一致を入れること。U+FFFD への置換規則により
`docs/\200.md` と `docs/\201.md` は同じ key になり、この衝突も「削除 + 追加」の
形をしているため、種別だけでは本物の置換と区別できない。
name が違う衝突は種別なしにする。どちらの種別を採ってももう一方の行に嘘の
アイコンが出るので、曖昧なら出さない。
