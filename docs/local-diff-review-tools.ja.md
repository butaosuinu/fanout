...
各行の先頭に変更種別(新規追加・変更・削除・移動)のアイコンを出す。
追加・削除行数からは判定できない — 空ファイルの追加は `+0 -0`、全行書き換えは
`+N -N` になる。
種別はサーバーから受け取らない。`/api/diff` の `files[]` は種別を持たず、足せば
1 MiB の応答上限に対するバイト圧と `--name-status` の追加呼び出しが要る。
patch は `new file mode` / `deleted file mode` / `rename from` を持っており、
ライブラリのパーサが解釈済みの `type` を出すので、それを畳んで使う。
`rename-pure` と `rename-changed` は「移動」1 種にする。内容が変わったかは行の
`+N -M` が既に示している。
同 path の 2 entry(file type change)は置換として「変更」に畳む。先頭は削除側
なので、そのまま採ると「削除された」と読める。
ただし畳めるのは、2 つの entry が同じ file だと言い切れるときだけ。
不正 UTF-8 の byte は 2 段階で U+FFFD へ潰れる — `core.quotePath` が既定(on)なら
patch の 8 進エスケープを `unquoteGitPath` が、off なら raw byte を Go の
`encoding/json` が潰す。
どちらの経路でも `docs/\200.md` と `docs/\201.md` は同じ文字列になり、この衝突も
「削除 + 追加」の形をしているため種別だけでは本物の置換と区別できない。
判定は「正規化前の name が一致し、かつ U+FFFD を含まない」。
満たさなければ種別なしにする。どちらの種別を採ってももう一方の行に嘘のアイコンが
出るので、曖昧なら出さない。
アイコンは `aria-hidden` にして、種別は行の accessible name に**後置**する。
前置すると支援技術の要素一覧の type-ahead が先頭から照合するため、40 行が
「変更 …」で始まってパスで絞り込めなくなる。
アイコンの列は種別が引けない行でも幅を確保する。行の左インデントはこの列が
担っているので、要素ごと畳むとその行だけ左へ飛び出す。
...
保存の 1 件は「この path のこの内容を見た」であり、同じ path が複数の fingerprint を
持ちうる。
1 path 1 fingerprint にすると、2 タブで同じ行を開いて片方だけ再取得したとき、古い
patch を見ているタブの操作が新しいほうのチェックを消す — どちらの「見た」も本当
なので、片方を捨てる理由が無い。
外すときも fingerprint 単位で消す。
内容が行き来する file 1 つに 500 件の枠を食わせないよう、1 path あたり新しい
ほうから 4 件で切る。
...
上限に当たったら捨てるのは**古いほう**である。
読み出し側だけで切ると、上限に達した瞬間から新しいチェックが黙って保存されなく
なる(書けはするが、次の読み出しで先頭 500 件に負けて消える)。

上限は 2 段で、先に 1 path あたりの fingerprint、次に path の本数を絞る。
500 は entry ではなく **path** の上限にすること。
entry で切ると、500 file 全部を確認済みにしたあとに 1 file が変わって再チェック
されるだけで、その履歴 1 件が枠を食って無関係な未変更 file のチェックが落ちる。
path 単位で落とすなら、ある path の履歴が別の path の現行 entry を押し出さない。
scope は 50 本まで残し、超えたら最終更新(`t`)の古いほうから捨てる。
数本で切ってはいけない — fan-out は親 1 つに子が何十個も並ぶので、隣の子を
レビューしている間に前の子の確認済みが消える。
剪定は書き込みの**前**に走らせる。後にすると、quota で書けなかった直後に領域を
空けて終わり(誰も書き直さない)になり、古い scope を捨てただけで終わる。
`writeLocal` は成否を返し、失敗したら最古の scope を 1 本落として一度だけ retry
する。
...
材料は `type` / `name` / `prevName` / `newObjectId` / `mode` / `prevMode` と
描画対象の全行で、FNV-1a に畳み込む。
`newObjectId`(patch の `index` 行)単独では足りない — pure rename は
`similarity index 100%` だけで `index` 行を持たない。
`mode` を外してもいけない。chmod は `index` 行も hunk も持たないので、
`+x` とその取り消しが同じ値になり、確認済みが残ったまま「隠す」で二度と出てこない。
`prevObjectId` と hunk の `@@` 行も混ぜる。rebase や merge-base の更新で、最終 blob が
同じまま同じ置換が別の箇所へ移ることがあり、new 側と行の中身だけでは区別できない。

fingerprint は 32bit を 2 本(別乗数)の 64bit にする。1 本では、500 files を何度も
再取得するうちに偶然の衝突が現実的な確率で起きはじめる。
ただしここで守れるのは偶然までで、**故意に衝突させる相手は守れない**。暗号学的
digest ではないので、patch を書く側が両方の版を選べるなら誕生日攻撃で衝突を作れる
(連結ハッシュは連結した幅ぶんの強度を持たない)。
ブラウザの同期 API に暗号学的 digest は無く、`crypto.subtle` は非同期なので、patch と
同じ render で fingerprint を出せなくなる。
...
`diff.ts` の変更種別より厳しく、1 entry でも U+FFFD を含むなら降ろす。
...
畳んだままになる。
「畳まれているか」の判定は 1 箇所(`useDiffCollapse` の `isCollapsed`)に置く。
描画側にも同じ式を書くと、上書き・確認済み・初期方針の優先順位が 2 箇所に増え、
片方だけ変えたときにボタンと本文が食い違う。
上書きを捨てる範囲は patch ではなく「session 行 + patch」で区切る。同じ worktree を
指す行(attached-agent など)は patch が一致するので、patch だけだと行を切り替えても
前の行の上書きが残り、確認済みで復元した file が開いたまま出る。
チェックの操作は、付ける側も外す側も上書き(`overrides`)を**消す**。
畳むかどうかは `collapsedAt` が確認済みから導くので、上書きを書く必要が無い。
`true` を書いてはいけない — 上書きは確認済みより優先するので、別タブで外されたときに
チェックだけ外れて本文が畳まれたまま残る。
`false` も書けない — 1,000 行超で既定折りたたみだった file が、開いたことすら無い
状態から全開になり、読んでいた位置が飛ぶ。
消すだけで、明示的に展開済み(上書き `false`)の file を確認済みにしたときもちゃんと
畳まれる。

行へ降ろすハンドラは identity を固定する(`useStableCallback`)。
毎 render 変わると `DiffFileRow` の memo が全件で外れ、チェック 1 回が
契約上限 500 files 全部の `setOptions` / `render` に化ける。
memo が効かないなら memo を置く意味も無い。
...
本文が空になったら理由を出すが、「すべてのファイル」とは言わない。
`plan` は patch を持つ file しか含まないので、バイナリやサイズ超過で省略された
file がある diff では嘘になる。

隠す設定でチェックを入れると、そのチェックごと unmount される。
フォーカスが `<body>` へ落ちてトラップの外に出るので、残っているうちの先頭の
チェックへ渡す(無ければオーバーレイ自身が引き取る)。
オーバーレイの先頭へ戻してはいけない — キーボードだけで読む人は file ごとに
ヘッダのボタン全部とサイドバーを Tab で辿り直すことになる。
DOM は commit 後に入れ替わるので、探すのは次フレーム。
行が消える理由はローカル操作だけではない。別タブが同じ scope でチェックすれば
storage 経由でこちらでも消えるので、隠れる集合が動いたあとに焦点が落ちていないかを
見て拾い直す。覆っているあいだだけ効かせること — コンパクトで背面を触っている人から
フォーカスを奪わない。

書き戻すのは、いま画面に出ている内容の entry 1 件だけにして、書く直前に storage を
読み直す。
呼び出し側が持っている一覧を丸ごと書くと、別タブが直前に入れたチェックを
巻き戻す — `storage` イベントの到着はこちらの操作より遅れうるので、購読だけでは
この競合を塞げない。
同じ組を入れ直すときは末尾へ動かす。並びがそのまま剪定順なので、動かさないと
「いちばん新しいチェック」が先に落ちる側に残る。

読み直してから書くまでのあいだに別タブが書き込む競合は、意図して塞いでいない。
localStorage に compare-and-swap は無く、塞ぐには Web Locks(非同期)へ移すしか
ないが、そうすると書き込みが 1 tick 遅れ、同じ file を続けて押したときの順序という
現実に起きる問題を、マイクロ秒の窓と引き換えに持ち込むことになる。
失われるのは 2 タブが同じ窓でチェックしたときの片方 1 件で、画面には即座に反映
されるので押し直せる。塞いだのは「古い snapshot で丸ごと上書きする」ほうの、
決定的に起きる取りこぼしである。
