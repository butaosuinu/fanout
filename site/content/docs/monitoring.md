...
After this popup successfully launches a Prompt, plan coordinator, or Issue Session, the console focuses the first newly created pane in actual creation order. Press `F11` or `prefix + T` to return. Agent attach (`a`), shell (`A` / `t`), watcher, and ordinary CLI launch paths leave focus where it was.

**Prompt** is the classic manual pane. Write a multi-line prompt and set the `claude` / `codex` launch counts; in the prompt field, `Shift+Enter` or `Ctrl+J` inserts a newline and `@` completes repository file paths. Manual `codex` panes start in Codex Plan Mode through app-server regardless of `codexPlanMode`; `claude` starts normally. Enabling the plan fan-out checkbox below switches the launch to a single coordinator (select exactly one agent) that decomposes the prompt into parallel tasks with `fanout plan` — the coordinator always launches as a normal agent, even `codex`.

**Issue** lists the repository's OPEN issues and lets you narrow them by number, title, or label. `Ctrl+O` opens the selected issue in the default browser. Pick an issue, choose the default `claude` / `codex` agent, and `Enter` opens an assignment screen that flips the agent per child of that issue — the equivalent of repeatable `--agent NUM=name`. An issue with OPEN children fans out the equivalent of `--unblocked-only`, leaving blocked children deferred. With `codexPlanMode` enabled, all children in that launch must be assigned to `codex`; a mixed assignment is rejected before any pane is created. An issue without children starts as a normal single pane under `@watch`, unaffected by `codexPlanMode` (see [Watcher]({{< relref "/docs/watcher" >}})).

The same plan fan-out checkbox from Prompt mode appears here for a single issue: turn it on and the child-assignment screen is skipped, launching one normal-mode coordinator pane that decomposes the issue into issue-less `fanout plan` tasks run by the chosen task agent. This checkbox is separate from `codexPlanMode`: the checkbox creates a coordinator and new tasks, while `codexPlanMode` changes how existing issue / Project children start. The checkbox is grayed out while the selected issue has OPEN children — fan those out instead.
