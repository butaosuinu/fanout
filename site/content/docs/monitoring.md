...
| Key | Action |
|---|---|
| `n` | Open a modal to create manual agent panes from a required **multi-line** prompt, `claude` / `codex` launch counts, and an optional slug. `codex` starts in Codex Plan Mode and receives the modal prompt inline; `claude` starts normally. Use `Up` / `Down` to pick an agent row, `Space` to toggle it, and `Left` / `Right` to change the count. In the prompt field, `Ctrl+J` inserts a newline; `Shift+Enter` works only when enhanced keyboard input is enabled with `FANOUT_TUI_ENHANCED_KEYS=1` and the terminal reports that key distinctly. `Enter` creates the selected panes. Manual panes are recorded as synthetic `@manual` state entries and appear in the list after launch. |
