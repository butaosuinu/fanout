...
| `n` | Open the new-session form (tmux popup or inline Herdr form). Its Mode row switches between Prompt and Issue. In a foreign or `default` Herdr console, this is the only action that can create work; see [New session modes](#new-session-modes). |
...
| `Z` | On tmux, focus the selected pane and zoom it. Creating or closing a pane, or resizing the window, unzooms it — press `Z` again if you need to. Disabled for Herdr rows with a reason. |
...
After this popup successfully launches a Prompt, plan coordinator, or Issue Session, the console focuses the first newly created pane in actual creation order. For an Issue fan-out, tmux creates the orchestrator first; Herdr creates it after the child panes, so the first new child receives focus. From a foreign or `default` Herdr console, opening or cancelling the form creates nothing; submit creates the work in fanout's repository-owned session and directly attaches that first terminal. Detach to return to the same TUI. If direct attach fails, the TUI reports it separately and keeps the created panes and state rows. Press `F11` or `prefix + T` to return from tmux. Agent attach (`a`), shell (`A` / `t`), watcher, and ordinary CLI launch paths leave focus where it was.
...
- A herdr row matches its saved identity against `herdr api snapshot` for liveness and agent state. The conversation is evidence, not the fence: the first unique valid reference from the expected provider is persisted under the owning state lock, and a later reference the same provider issued in the same pane replaces it, because a provider restarts its conversation in place (Claude's `/clear`, Codex's `/new`). The same restart leaves the agent record unnamed, which the row also admits; fanout re-asserts the name on the next focus, send, or close. `fanout msg nudge` addresses the pane through a narrower target and leaves the name as it found it. A conversation from a different provider, and an agent record answering to another name, are still refused. No other identity field is filled in from the snapshot. Output peek returns content only for a live row in this repository's fanout-owned Herdr session.
