...
Review state filters separately from PR state, because the two are different questions: `pr:open` asks where the PR sits in its lifecycle, `review:approved` asks whether it passed review. An approved PR still matches `review:approved` after it merges. `review:` takes `approved`, `changes-requested`, `review-required`, and `none`.
