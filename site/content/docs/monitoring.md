...
| Key | Action |
|---|---|
| `n` | Open a modal to create a manual agent pane from a required **multi-line** prompt, a `claude` / `codex` agent choice, and an optional slug. `codex` starts in Codex Plan Mode; `claude` starts normally. In the prompt field, `Shift+Enter` inserts a newline (`Ctrl+J` is a fallback for terminals that do not distinguish `Shift+Enter`); `Enter` creates the pane. Manual panes are recorded as synthetic `@manual` state entries and appear in the list after launch. |
