...
| `a` | Attach one or more agent panes to the selected row's recorded worktree. No git worktree is created. The attached rows share the selected worktree and branch, can be focused and peeked, and do not count toward merge progress. `codex` starts in [Codex Plan Mode]({{< relref "/docs/agent-integrations#codex-plan-mode" >}}). |
...
The same plan fan-out checkbox from Prompt mode appears here for a single issue: turn it on and the child-assignment screen is skipped, launching one normal-mode coordinator pane that decomposes the issue into issue-less `fanout plan` tasks run by the chosen task agent. The checkbox creates a coordinator and new tasks; child launch posture applies to those tasks as well as existing issue / Project children. The checkbox is grayed out while the selected issue has OPEN children — fan those out instead.
