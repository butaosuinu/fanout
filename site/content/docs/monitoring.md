...
Files are expanded with syntax highlighting by default. Only a file with 1,000 or more changed lines starts collapsed, and expanding it keeps the highlighting — except for very large files, which render as plain text: highlighting is dropped above 150,000 changed characters or 20,000 changed lines, because tokenizing runs over the whole file rather than the visible rows. The viewer renders just the visible rows, so a diff of several thousand lines stays responsive. The file name stays pinned to the top while you scroll through that file, and long lines wrap. When deletions and additions are side by side, a file with only additions or only deletions is shown on one side instead of the full width.

It opens compact by default: a panel beside the detail drawer. Drag its left edge to widen it up to 95% of the window, over the drawer. On a window wider than 1,100px the Session list and the drawer stay usable in compact, so you can follow a pane's output while reading its diff; below that the panel fills the width under the nav and behaves like full screen. The diagonal-arrow icon switches between compact and full screen. The mode and the width are stored in the browser.
