...
| `--backend` | `<tmux\|herdr>` | Runtime backend for this run. Default: `tmux`. The [herdr backend]({{< relref "/docs/herdr-backend" >}}) launches issue and Project children in a repository-owned session. A parent with recorded panes keeps its recorded backend; a conflicting override fails instead of mixing backends. Herdr rejects `--team` and Codex child Plan Mode before mutation. |
