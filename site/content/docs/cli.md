...
| `--codex-plan-mode` / `--no-codex-plan-mode` | — | For `--agent codex`, start an app-server Plan Mode turn and attach an interactive Codex TUI instead of positional `codex "<prompt>"`. Default: off. Details in [Agent Integrations]({{< relref "/docs/agents" >}}). |
