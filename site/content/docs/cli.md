...
- `fanout <parent> --close <NUM>` verifies the recorded tmux pane by `shellKey` or `worktreePath`, closes and rechecks it, then removes the worktree with `git worktree remove <path> --force`, removes the state entry, and runs `git worktree prune`. If pane ownership cannot be verified or the pane remains live, fanout keeps the worktree and state entry for a retry.
