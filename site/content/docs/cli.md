...
| `--codex-plan-mode` / `--no-codex-plan-mode` | — | For `--agent codex`, start an interactive Codex TUI through app-server and type the prompt into the composer via `/plan` instead of positional `codex "<prompt>"`. Default: off. Details in [Agent Integrations]({{< relref "/docs/agents" >}}). |
