...
| `--backend` | `<tmux\|herdr>` | Runtime backend for this run. Default: `tmux`. The [herdr backend]({{< relref "/docs/herdr-backend" >}}) launches issue and Project children in a repository-owned session, including Codex child Plan Mode through fanout's app-server controller. A parent with recorded panes keeps its recorded backend; a conflicting override fails instead of mixing backends. |
...
merged PR. With herdr, once no task rows remain for that plan, it also closes the
coordinator workspace and retires its state row and intent. An auxiliary pane
in that workspace requires manual cleanup. These modes honor `FANOUT_STATE_PATH`.
...
Sibling coordination over a per-parent SQLite message bus. Run from inside a fanned pane: `fanout msg` auto-detects which child and parent you belong to from the runtime pane or managed worktree and the owning checkout's `.fanout/state.json`. Panes opt in with [`--team`](#run-control-flags) at fan-out time, but any pane can `register` itself afterward. How this differs from Claude Code Agent Teams, and the coordination workflow, are covered in [Workflow]({{< relref "/docs/workflow" >}}).
...
| `nudge` | `<N>` — best-effort: drop an inbox hint into peer `#N`'s pane through its recorded runtime only when its agent can take queued input (state `running` / `working` / `plan` / `idle`). tmux uses `send-keys`; herdr issues one no-wait `agent prompt` only after fresh refined telemetry and live pane, worktree, agent, and process identity match. A notify verb, not a message: it never touches the DB and is a no-op success otherwise (pane gone, state unknown, `blocked` on a permission prompt, or done). Agents without current pane-state refinement are excluded. |
...
The database lives at `/tmp/fanout-<repo>-<parent>.db` and is overridable with `FANOUT_DB_PATH`; herdr `--team` requires that override to be absolute so the launcher and child worktree cannot open different files. Messages persist in the DB and a sibling reads them at its own checkpoints; with `--team`, `claude` panes and fresh non-Plan `codex` panes also get a push lane that delivers them as they arrive: `claude` panes are briefed to run `fanout msg watch` under the Monitor tool, and fresh non-Plan `codex` panes start through an app-server bridge that injects unread messages into an idle turn as quoted, untrusted data (Codex Plan Mode panes stay pull-based). Neither lane writes to pane input — `nudge` is the only verb that does. Without a working lane (no Monitor, a restored codex pane), siblings fall back to pull; after an injection failure, recover with `inbox --all` because the failed batch is already marked read. A codex bridge that fails to start (a bad `FANOUT_DB_PATH`, wrong DB ownership or permissions) is not a fallback case — that child launch fails. The pure-Go SQLite driver is embedded, so no external `sqlite3` is required.
...
| `shutdown` | Retire an empty owned server. Refuses while a child herdr row remains in this repository's state (every linked worktree counts), while the session still holds a workspace, or while an unprunable herdr intent is pending. After proving the workspace snapshot is empty, removes the console and coordinator scaffold rows. It also prunes a `realized` intent when the snapshot has no workspace with its recorded label and any required checkout is absent; a remaining fanout-created branch must still point to its saved base SHA before shutdown deletes it. Exit console and coordinator shells first. |
...
| `FANOUT_DB_PATH` | Override the per-parent peer-messaging SQLite path used by `--team` and `fanout msg`. herdr `--team` requires an absolute path. Default: `/tmp/fanout-<repo>-<parent>.db`. |
