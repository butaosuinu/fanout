...
- `childPlanMode`(既定 `false`): issue / Project の子、OPEN 子なし issue の単独 Session、`fanout plan` のタスク、watcher 起動。on にすると無人の [watcher]({{< relref "/docs/watcher" >}}) Session が plan 承認待ちで止まります(v2.1.207 未満または version 判定不能の claude は警告つきで mode フラグが省かれ、止まりません)。herdr backend の Codex Plan Mode は fanout の app-server controller と owned launcher で動きます。
