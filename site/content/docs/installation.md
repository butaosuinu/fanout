...
These three cover the default tmux backend. The opt-in [herdr backend]({{< relref "/docs/herdr-backend" >}}) — observation-only in v1 — swaps tmux for herdr 0.7.3, which is AGPL-licensed and not bundled with fanout; install it separately.
...
curl -fsSL https://raw.githubusercontent.com/butaosuinu/fanout/main/install.sh | FANOUT_VERSION=v0.13.0 sh
```
