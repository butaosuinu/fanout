...
These three cover the default tmux backend. The opt-in [herdr backend]({{< relref "/docs/herdr-backend" >}}) swaps tmux for stable herdr 0.7.5 or newer on CLI launch lanes. herdr is not bundled with fanout; install it separately. v0.8.0 and later are Apache-2.0; 0.7.x is AGPL-3.0 with a commercial option.
...
curl -fsSL https://raw.githubusercontent.com/butaosuinu/fanout/main/install.sh | FANOUT_VERSION=v0.17.0 sh
```
...
- `$CODEX_DIR/skills/` — the same six skills; `post-work-review` bundles its marker helper inside the skill
