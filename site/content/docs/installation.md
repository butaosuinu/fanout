...
These three cover the default tmux backend. The opt-in [herdr backend]({{< relref "/docs/herdr-backend" >}}) — observation-only in v1 — swaps tmux for stable herdr 0.7.4 or newer, subject to its schema and command-surface gates. herdr is AGPL-licensed and not bundled with fanout; install it separately.
