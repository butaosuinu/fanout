...
`shutdown` also recovers a `realized` intent left by a failed launch. It prunes the intent only when an owned-workspace snapshot has no workspace with the recorded label; worktree and resume intents additionally require the checkout to be absent. A failed snapshot, a matching workspace, or a remaining checkout keeps the intent and refuses shutdown.
