...
そのままです。
ただし非常に大きなファイルはプレーンテキストで描画します(変更 150,000 文字または
20,000 行を超えるとハイライトを落とします。トークン化は表示中の行ではなくファイル
全体に走るためです)。
...
幅 1,100px を超える画面では、コンパクト中も Session 一覧やドロワーをそのまま
触れるので、ペインの出力を見ながら差分を追えます。
それ以下の幅ではパネルが nav 下の全幅になり、全画面と同じ扱いになります。
