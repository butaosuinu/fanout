...
hook は PR の base と review marker metadata の `base=` も照合します。
`/post-work-review` の最終ステップでレビュー済みのコミットと base を記録してから、`gh pr create --base <reviewed-base>` を実行してください(一度だけバイパスしたいときは、コマンドの先頭に次を付けます)。
