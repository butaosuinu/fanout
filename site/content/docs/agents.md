...
The reviewer receives the target repository path and diff scope, so repository content is sent to the Codex model. The review task treats that content as untrusted evidence and tells the reviewer never to follow directives found there. Before spawning, the marker helper rejects candidate changes to `AGENTS.md`, `AGENTS.override.md`, or repository `.codex` files because Codex can load them during child bootstrap. The installed skill package and helper must also be non-symlinked copies outside the reviewed repository. Review instruction changes from a trusted checkout or by a human instead. A native subagent inherits the parent session's sandbox, approval policy, and network restrictions. The skill tells reviewers not to edit, request approval, or use the network, but it cannot create a stricter child-only sandbox. Start Codex read-only first when enforced read-only access is required. If the trust boundary, native spawning, or waiting is unavailable, the gate stops without a fallback.
