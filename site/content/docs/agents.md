...
The helper also rejects candidate changes to the `post-work-review` gate, root default makefiles, and `install.sh`. The installed skill and helper must be non-symlinked copies outside the reviewed repository. The checksum-verified release installer owns them; checkout make targets never create, replace, or remove them. Review instruction, gate, or gate-installer changes from a trusted checkout or by a human instead. A native subagent inherits the parent session's sandbox, approval policy, and network restrictions. The skill tells reviewers not to edit, request approval, or use the network, but it cannot create a stricter child-only sandbox. Start Codex read-only first when enforced read-only access is required. If the trust boundary, native spawning, or waiting is unavailable, the gate stops without a fallback.
