...
- 行が保存した conversation は、同じ provider が同じ pane で新しい会話を始めたとき (claude の `/clear`、codex の `/new`) にそちらへ追随します。focus、peek、`--close`、`--cleanup` はその前後で使えたままです。別の provider の conversation や、runtime が発行していない ref は従来どおり拒否します。
