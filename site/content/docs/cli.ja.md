...
| `--codex-plan-mode` / `--no-codex-plan-mode` | — | `--agent codex` のとき、positional の `codex "<prompt>"` ではなく Codex app-server 越しの interactive Codex TUI に `/plan` として prompt を入力する。既定: off。詳細は[エージェント連携]({{< relref "/docs/agents" >}})。 |
