...
as a broad reviewer. The sole task input is the absolute path to one
driver-generated `verify_bundle`. Read that file, then consult repository files
only when needed to verify its scoped fix. Do not use implementation history,
main-agent reasoning, chat summaries, or prior review conclusions beyond the
findings recorded in the bundle.
