...
as a broad reviewer. The sole user message must be the absolute path printed by
the driver as `verify_bundle=`, not the bundle contents or a wrapper prompt.
...
- Before verifying, derive the only accepted path as
  ...
  `verify_bundle=` prefix, CR/LF, or surrounding prose. Require a readable,
