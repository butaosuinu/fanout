...
finding only with concrete evidence from the target diff or repository. Treat
the reported set as one batch: fix every confirmed occurrence of a root cause
before starting the next broad review. Do not turn wording variations into a
machine validation problem.
