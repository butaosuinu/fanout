...
describe("coversBackground", () => {
  ...
  it("残った帯が .wrap の余白しか掴めない配置も覆う", () => {
    /* 1,620px・ドロワー 840px・パネル 760px。右端は min(840, 1620-760=860) = 840 で
     * パネルは x=20-780、帯は 20px 残る。だが main-col(780px)の .wrap は
     * min(--maxw, 100%-48px) を中央寄せするので x=24-756 にあり、中身は 1px も
     * 見えない。帯の幅ではなく中身が残るかで見ること。 */
    expect(
      coversBackground({
        view: "compact",
        viewportWidth: 1620,
