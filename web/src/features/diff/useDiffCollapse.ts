...
/* いま折りたたまれているか。上書きが無ければ初期方針(planDiffFiles)に従う。 */
function collapsedAt(entry: DiffFilePlan | undefined, override: boolean | undefined): boolean {
  return override ?? entry?.initiallyCollapsed ?? false;
}
...
export function useDiffCollapse(patch: string, plan: DiffFilePlan[]) {
  ...
  const onToggle = useCallback(
    (i: number) => setCollapsed(i, !collapsedAt(plan[i], overrides.get(i))),
    [overrides, plan, setCollapsed],
  );
  /* 折りたたまれていれば開く。すでに開いていれば何もしない — 無条件に書くと
   * 同じ状態のまま Map を差し替えて再レンダーを起こす。 */
  const expand = useCallback(
    (i: number) => {
      if (collapsedAt(plan[i], overrides.get(i))) setCollapsed(i, false);
    },
    [overrides, plan, setCollapsed],
  );
  ...
  return { overrides, onToggle, expand, onExpandAll, onCollapseAll };
}
