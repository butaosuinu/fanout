...
func TestStateCompatDecodesLegacyTmuxRow(t *testing.T) {
	...
	for _, tt := range []fieldCase{
		...
		{name: "herdrConsoleShell stays empty when the key is absent", got: legacy.ConsoleShell, want: ""},
...
func herdrRowFields(p Pane) []fieldCase {
	return []fieldCase{
		...
		{name: "reported_state_sequence", got: p.ReportedStateSeq, want: uint64(7)},
		...
		{name: "herdrConsoleShell", got: p.ConsoleShell, want: "/bin/zsh"},
