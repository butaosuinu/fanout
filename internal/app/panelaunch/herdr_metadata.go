...
// reportManagedSidebarMetadata publishes the display-only tokens once the launch
...
func managedSidebarMetadata(req Request, intent state.LaunchIntent) backend.MetadataReport {
	return backend.MetadataReport{
		...
		WorkspaceTokens: []backend.MetadataToken{
			{Name: metadataIssueToken, Value: managedChildToken(req)},
			{Name: metadataSlugToken, Value: managedMetadataValue(req.Slug)},
		},
		...
		PaneTokens: []backend.MetadataToken{
			{Name: metadataParentToken, Value: managedParentToken(req)},
...
// managedChildToken names the child: "#<issue>" for issue and Project children,
// the task ID for a plan task. Synthetic negative rows have neither and clear.
func managedChildToken(req Request) string {
	if req.Number > 0 {
		return "#" + strconv.Itoa(req.Number)
	}
	return managedMetadataValue(req.TaskID)
}

// managedParentToken names the fan-out the pane belongs to. A watcher launch
// stores the synthetic @watch parent, and the rest of the launch path resolves
// it to the issue the watcher picked up — managedCoordinatorRuntimeParent does
// exactly this for the coordinator workspace — so the sidebar names that issue
// instead of the marker.
func managedParentToken(req Request) string {
	if canonicalManagedParent(req.ParentRef) == WatchParentRef && req.Number > 0 {
		return managedChildToken(req)
	}
	return managedMetadataValue(parentDisplay(req.ParentRef))
}

// managedMetadataValue shapes one value the way Herdr stores it — trimmed,
...
func managedMetadataValue(raw string) string {
