...
# 日本語表現だけで言及されている子などが対象。/fanout スキル経由で呼ぶと、
# エージェントが親本文を読んで候補を提示し、承認された番号をこの flag に載せる。
# CLI 直接利用時はここに番号を明示する。CLOSED や存在しない番号は警告して
# スキップ。--only / --skip と併用可（include で追加した後にフィルタ適用）。
