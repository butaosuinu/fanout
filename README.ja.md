...
各ポップアップに `/tmp/dmux-popup-<timestamp>.json` のパスを渡し、ポップアップ
...
- **エージェント名が解決できること**: `--agent <name>` を渡すか、呼び出し元
  ...
  必要とします。エージェントセッション内から `/fanout` を叩けば追加の flag
  なしで動きます。素のシェルから叩く場合は `--agent` 必須です。
...
6. 各対象 issue について:
   ...
   - `pgrep -f 'newPanePopup.js'` でポップアップ node プロセスを特定し、
     `ps -o args=` から `/tmp/dmux-popup-*.json` の resultFile パスを抽出、
...
- `--debug` を付けて再実行し、どの段階で失敗しているかを確認する。よくあるケース:
  - `newPanePopup did not appear within 5s` — `n` に dmux が反応していない。
    別のポップアップが既に開いていることが多いので、手動で `Esc` を押してから再実行。
  - `agentChoicePopup did not appear within 5s` — プロンプトポップアップは閉じたが
    エージェント選択ポップアップが出てこない。dmux 設定で少なくとも 1 つの
    エージェントが有効になっているか確認する。
...
- **IPC の経路**。検出には tmux セッションオプション
  ...
  resultFile（`/tmp/dmux-popup-*.json`）への atomic 書き込みを使います。
