...
- dmux プロジェクト設定で **有効化されているエージェントが 1 つだけ** であること、
  あるいは `--agent` を渡すこと。複数のエージェントが有効だと dmux はポップアップを
  表示しますが、fanout はこれをベストエフォートでしかナビゲートできません
  （エージェント名の 1 文字目を送るだけ）。
...
# 複数エージェントが有効なときにエージェント名をヒントとして渡す（ポップアップのベストエフォートナビ）
...
TUI がペイン一覧画面にあること、エージェントは 1 つだけ有効（または `--agent`
を渡す）。詳しくは **前提条件** と **トラブルシューティング** を参照してください。
...
6. 各対象 issue について:
   ...
   - コントロールペインに `Escape`、`n`、1 行プロンプト
     ...
     続けて `Enter` を送る。
   - `--agent X` が渡されていれば、X の 1 文字目と `Enter` を送ってエージェント
     ポップアップのナビゲートを試みる。
