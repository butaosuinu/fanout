...
fanout は 1 つの Go バイナリに、エージェント連携ファイルを加えた構成で配布
されます。Claude Code にはスラッシュコマンド + スキル群、Codex CLI には
スキル群を用意しています。最も簡単な経路はビルド済みバイナリを
[GitHub Releases](https://github.com/butaosuinu/fanout/releases)
（darwin / linux × amd64 / arm64）から取って `PATH` に置き、リポジトリを
クローンして `make install` を実行することです。

ソースから一括ビルド + インストールする場合:

```bash
make install        # Go バイナリをビルドし、CLI + Claude/Codex 連携を ~/.local, ~/.claude, ~/.codex にコピー
...
Go ツールチェインがあれば、バイナリだけ入れる手もあります:
...
make test           # build + Tier 1 + Tier 2 黒箱テスト (bats-core 必須)
...
`--dry-run` 出力を意図的に変更した場合は `FANOUT_GOLDEN_UPDATE=1 make test-tier2`
で golden を再生成してください。Tier 3 (live dmux E2E) は手動運用のままです。

## 前提条件

- `gh` CLI、`tmux`、`pgrep`、`gh-sub-issue` 拡張
...
1. `gh`、`tmux`、`pgrep`、`gh-sub-issue` がインストールされているかを確認。
...
それでも文字化けするなら、`dmux.config.json` にそのまま保存されているかを
確認してください。書き込まれる JSON を事前に確認するには `--dry-run` を使います。
