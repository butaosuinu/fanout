...
// launch transaction.
func launchIssueOrchestratorPrepared(projectRoot, session, commandName string, runtimeBackend backend.Backend, managed panelaunch.ManagedSessionRuntime, store state.Store, recorder panelaunch.StateRecorder, hookConfig hooks.Config, issue ghissue.Issue, agentName string, orchestratorPlanMode bool) (panelaunch.Request, panelaunch.Result, bool, string, error) {
	var fallbackNotice string
	req, result, err := launchPlanCoordinatorLocked(projectRoot, session, commandName, runtimeBackend, managed, agentName, fmt.Sprintf("%d", issue.Number), store, recorder,
	...
	if errors.Is(err, errIssueOrchestratorRecorded) {
		return panelaunch.Request{}, panelaunch.Result{}, false, "", nil
	}
	if err != nil {
		return panelaunch.Request{}, panelaunch.Result{}, false, "", err
	}
	...
	return req, result, true, notice, nil
}
