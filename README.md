...
- tmux is the default runtime backend, and existing tmux workflows do not
  change. The opt-in herdr backend is observation-only in v1 and needs a
  running named session of herdr 0.7.3, which is AGPL-licensed and not
  bundled — install it separately. See the
