...
visible without agent instrumentation. Press `Enter` or `o` on a live row to
focus that pane, and press `p` to refresh the read-only output snapshot shown in
the detail panel. Rows whose recorded pane no longer exists in tmux are marked
`stale!` and are skipped by focus/peek actions. Press `q` to leave the console;
the tmux session and child panes are left running.
